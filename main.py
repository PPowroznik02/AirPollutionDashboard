...
def get_sensor_data(id, paramCodes):
    ...
    response = urlopen(url)
    data = json.loads(response.read()) # informacje o wszyskich czujnikach na danej stacji
    # print(json.dumps(data, indent = 4, sort_keys=True))
    ...
    for d in data:
        if (d["param"]["paramCode"] in paramCodes):
            try:
                ...
                time = date + "_"  + t;
                return (d["id"], d["param"]["paramCode"], val, date, time)
            except:
                pass

def write_to_csv(sensors, headers):
    with open('dane_z_sensorow.csv', 'w', newline='') as file:
...
@sched.scheduled_job('cron', minute=15, misfire_grace_time=3600)
def timed_job():
    try:
        ...
        param_codes = ["SO2", "PM10", "PM2.5", "NO2", "C6H6", "CO", "O3"]  # lista z wyszukiwanymi parametrami
        stations_id = list(stations.keys())  # zapisanie kluczy slownika do listy
        sensors = get_data_for_all_sensors(stations, stations_id, param_codes)  # wyciagniecie danych z kazdego sensora i zapisanie do slownika
        ...
        #logowanie do arcgis online
        gis = GIS("http://www.arcgis.com", logowanie["login"], logowanie["haslo"])
        table_url = "https://services2.arcgis.com/MzCtPDSne0rpIt7V/arcgis/rest/services/dane_z_sensorow/FeatureServer/0"


        tbl = FeatureLayer(table_url, gis=gis)
        tbl.manager.truncate()

        casos_csv = r'dane_z_sensorow.csv'
